...
/* MDK-ARM/armcc (AC5) cannot expand function-like macros inside #include,
 * so the platform header is included directly here (STM32 port note). */
#include "mqtt_port.h"
#endif
...
/* MQTT_ prefix avoids a name clash with the HAL ErrorStatus enum
 * (SUCCESS = 0U), which is a C enum constant and cannot be #undef'd. */
